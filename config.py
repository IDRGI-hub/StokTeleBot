TOKEN = "7432975813:AAENk2vwoT0CtAHv8Lj92Ow4qP9ilt1zRcA"	# bot token
CHAT_IDS = ["439716080", "857251907", "889953315"] # id чатов

# URL-адреса
MAYAK_URL = "https://app.mayak.bz/users/sign_in"
WILDBERRIES_URL_TEMPLATE = "https://www.wildberries.ru/catalog/{}/detail.aspx"

# Данные для входа
USERNAME = "89152251909"
PASSWORD = "D17102002o"

# Список товаров для парсинга
ARTICLES = {
    135845152: "машинка от катышек",
    254075751: "ми бенд 9",
    97717203: "ми бенд 7",
    303745709: "сушилка",
    306918588: "гриль",
    299192574: "отпар new",
    166744041: "отпар old",
    315338877: "G20 Lite",
    182168410: "G24",
    234336170: "X27G",
    283948163: "X34",
    234386201: "вертушка",
    252712311: "увлж 03",
    252712312: "увлж 05",
    252712313: "увлж 06",
    227649169: "21.45"
}

# Пути к файлам
COOKIE_FILE = "C:\StokTelebot\Parser\web_driver\cookies.json"
EXTENSION_PATH = "C:\StokTelebot\Parser\extension\extension.xpi"  # Укажи правильный путь к расширению
OUTPUT_FILE = "C:\StokTelebot\Parser\output.json"

# Логирование
LOGGING_LEVEL = "INFO"